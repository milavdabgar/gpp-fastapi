...
async def update_faculty_by_id(
    ...
    try:
        ...
        return DataResponse(
            ...
            data=FacultyResponse.from_orm(updated_faculty)
        )
    except AppError as e:
        raise HTTPException(
            status_code=e.status_code,
...
@router.delete("/{faculty_id}", response_model=ResponseBase)
...
@router.get("/department/{department_id}", response_model=DataResponse[List[FacultyResponse]])
async def get_faculty_by_department(
    ...
    """
    faculty_members = get_faculties_by_department(db, department_id)
    
    return DataResponse(
        status="success",
        data=[FacultyResponse.from_orm(faculty) for faculty in faculty_members]
    )